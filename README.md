...
**CI 平台矩阵（🔴 未激活）**：`.github/workflows/ci.yml` 已就绪，覆盖
`{ubuntu-latest, windows-latest} × {3.11, 3.12, 3.13}`（`fail-fast: false`）。
**owner 于 2026-09-11 决定暂不建立远程仓库，因此该工作流处于未激活状态、不会执行。**

> ⚠️ 三条边界（不得省略）：
> 1. 该工作流是**未激活配置**，不是运行中的 CI，也**不是已验证的 CI**；
> 2. 在建立远程并首次成功执行前，**不得宣称「CI 已通过」**，也不得据本文件推断
>    跨平台已受保护——**跨平台差异目前仍无机械化拦截**；
> 3. **Linux 侧从未真实执行过**，不得预称其会通过。
>
